...
public sealed class Lea256Cipher : LeaCipherBase
{
    ...
    protected override uint[,] GenerateRoundKeys(byte[] key)
    {
        ...
        for (int i = 0; i < RoundCount; i++)
        {
            ...
            T[idx0] = BitOperations.RotateLeft(T[idx0] + BitOperations.RotateLeft(d, i), 1);
            roundKeys[i, 0] = T[idx0];

            int idx1 = (baseIndex + 1) % 8;
            T[idx1] = BitOperations.RotateLeft(T[idx1] + BitOperations.RotateLeft(d, i + 1), 3);
            roundKeys[i, 1] = T[idx1];

            int idx2 = (baseIndex + 2) % 8;
            T[idx2] = BitOperations.RotateLeft(T[idx2] + BitOperations.RotateLeft(d, i + 2), 6);
            roundKeys[i, 2] = T[idx2];

            int idx3 = (baseIndex + 3) % 8;
            T[idx3] = BitOperations.RotateLeft(T[idx3] + BitOperations.RotateLeft(d, i + 3), 11);
            roundKeys[i, 3] = T[idx3];

            int idx4 = (baseIndex + 4) % 8;
            T[idx4] = BitOperations.RotateLeft(T[idx4] + BitOperations.RotateLeft(d, i + 4), 13);
            roundKeys[i, 4] = T[idx4];

            int idx5 = (baseIndex + 5) % 8;
            T[idx5] = BitOperations.RotateLeft(T[idx5] + BitOperations.RotateLeft(d, i + 5), 17);
